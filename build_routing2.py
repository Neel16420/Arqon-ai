"""Rewrites src/pages/Routing.tsx with all structural fixes."""
import pathlib
 ...
 * No absolute positioning of provider cards — no collisions possible.
 */
function FlowMap() {
  ...
  const recalc = () => {
    ...
    const nextLines = providerRefs.current.map(el => {
      if (!el) return { x1: ex, y1: ey, x2: ex, y2: ey }
  ...
  useEffect(() => {
    // Delay slightly so the DOM has painted
    const t = setTimeout(recalc, 80)
    window.addEventListener('resize', recalc)
    return () => { clearTimeout(t); window.removeEventListener('resize', recalc) }
  }, [])

  // Provider grid layout (6 cells around the center):
  // Row 0: [OpenAI(0)]  [empty]   [Google(2)]
  // Row 1: [empty]      [ENGINE]  [empty]
  // Row 2: [Anthropic(1)] [empty] [Mistral(3)]
  // Mobile: all stacked, engine first
  //
  // We use a fixed 3-col grid and assign each provider to a specific cell.
  const gridPositions = [
  ...
  return (
    <div ref={containerRef} className="relative w-full"
      ...
      {/* SVG lines layer — sits behind the grid, same dimensions */}
        ...
        {lines.map((l, i) => {
          ...
          return (
            <g key={p.id}>
              ...
              {/* animated dash */}
              <line x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2}
                    stroke={p.color} strokeWidth="1.5" opacity="0.55"
                    className="flow-line" />
            </g>
          )
        })}
      ...
      {/* Grid layout for nodes — 3 columns (provider | center | provider) */}
      <div className="relative grid grid-cols-3 grid-rows-3 gap-4"
           style={{ placeItems: 'center', minHeight: '320px' }}>

        {/* Providers — each fixed to its grid cell */}
        {PROVIDERS.map((p, i) => {
        ...
        {/* Arqon Engine — center cell (col 2, row 2) */}
        <div
          ref={engineRef}
          className="flex flex-col items-center gap-2 z-10"
          style={{ gridColumn: 2, gridRow: 2 }}
        >
          <div className="relative w-16 h-16 rounded-3xl flex items-center justify-center border shadow-lg"
            ...
            <div className="absolute inset-0 rounded-3xl animate-pulse"
                 style={{ background: 'rgb(var(--color-accent-rgb) / 0.05)' }} />
            <Zap size={24} className="text-accent relative z-10" />
          </div>
          <span className="text-[11px] font-bold text-foreground tracking-tight text-center"
