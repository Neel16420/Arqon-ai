      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 md:px-10 lg:px-12 space-y-8 custom-scrollbar">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className={`h-9 w-9 flex items-center justify-center bg-surface/50 border border-border/40 rounded-lg text-muted hover:text-foreground hover:bg-surface transition-colors ${
            ...
            <div className="flex items-center gap-2 border-l border-border/40 pl-3">
              <button
                onClick={() => exportBlob("csv")}
                className="h-9 px-3 flex items-center gap-2 bg-surface/50 border border-border/40 rounded-lg text-xs font-medium text-muted hover:text-foreground hover:bg-surface transition-colors"
              >
                <Download size={14} /> CSV
              </button>
              <button
                onClick={() => exportBlob("json")}
                className="h-9 px-3 flex items-center gap-2 bg-surface/50 border border-border/40 rounded-lg text-xs font-medium text-muted hover:text-foreground hover:bg-surface transition-colors"
              >
        ...
        {activeProviders.length === 0 ? (
          <EmptyState
            ...
            title="No Analytics"
            subtitle="Analytics will appear once requests begin."
          />
        ) : (
          <>
