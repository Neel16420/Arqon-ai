  ...
  for (let i = 0; i < 80; i++) {
    ...
    entries.push({
      ...
      requestPreview:
        '{"model":"' +
        model +
        '","messages":[{"role":"user","content":"Summarize the quarterly report and identify key trends..."}],"max_tokens":1024}',

      responsePreview:
        status === "success"
          ? '{"id":"chatcmpl-abc123","choices":[{"message":{"role":"assistant","content":"The quarterly report highlights three major trends: 1) Revenue grew 18% YoY..."}}],"usage":{"prompt_tokens":142,"completion_tokens":312}}'
          : '{"error":{"type":"' + status + '","message":"' + errors[0] + '"}}',
    })
  }
...
export default function Logs() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterProvider.length)
      data = data.filter((l) => filterProvider.includes(l.provider))

    if (filterStatus.length)
      data = data.filter((l) => filterStatus.includes(l.status))

    data.sort((a, b) => {
      const va =
        sortBy === "timestamp" ? new Date(a.timestamp).getTime() : a.latency

      const vb =
        sortBy === "timestamp" ? new Date(b.timestamp).getTime() : b.latency
  ...
  return (
    <div className="flex gap-0 h-full relative">
      {/* Main content */}
      <div
        className={`flex flex-col gap-4 flex-1 min-w-0 transition-all duration-200 ${
          selected ? "lg:mr-96" : ""
        }`}
      >
        {/* Filters */}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {PROVIDERS.map((p) => (
              <button
                key={p}
                onClick={() =>
                  toggleFilter(filterProvider, p, setFilterProvider)
                }
                className="h-7 px-2.5 rounded-full text-xs font-medium transition-all"
                style={{
                  background: filterProvider.includes(p)
                    ? "rgb(var(--color-accent-rgb) / 0.12)"
                    : "var(--color-surface-2)",

                  border: filterProvider.includes(p)
                    ? "1px solid rgb(var(--color-accent-rgb) / 0.3)"
                    : "1px solid var(--color-border)",

                  color: filterProvider.includes(p)
                    ? "var(--color-accent)"
                    : "var(--color-muted)",
                }}
              >
            ...
            {STATUS_OPTIONS.map((s) => (
              <button
                ...
                style={{
                  ...
                  border: filterStatus.includes(s)
                    ? "1px solid rgb(var(--color-accent-rgb) / 0.3)"
                    : "1px solid var(--color-border)",
              ...
              <tbody>
                {paginated.length === 0 ? (
                    ...
                    <td colSpan={8} className="py-4">
                      {hasFilters ? (
                        <EmptyState
                          ...
                          subtitle="No results match your current filter criteria."
                        ...
                        <EmptyState
                          ...
                          title="No Logs Yet"
                          subtitle="Requests will appear here once traffic flows through Arqon."
                        />
                      )}
                    </td>
                  </tr>
                  ...
                  paginated.map((row, i) => (
                    <tr
                      key={row.id}
                      className={`group cursor-pointer animate-fade-in-up transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm [&>td:first-child]:rounded-l-xl [&>td:last-child]:rounded-r-xl relative ${
                        selected?.id === row.id ? "shadow-sm" : ""
                      }`}
                      style={{
                        ...getStaggerDelay(i),
