  ...
  return (
    <div className="w-full flex items-center justify-between gap-8 h-full">
      ...
      <div className="flex items-center gap-3 w-[250px] shrink-0">
        <span className="text-[10px] font-medium text-muted uppercase tracking-widest mr-2">
          Route Flow
        </span>
      ...
      <div className="flex-1 flex justify-center min-w-0">
        <div className="flex items-center max-w-sm w-full relative">
          ...
          {/* Progress bar */}
          <div
            className="absolute top-1/2 left-0 h-[1px] bg-success -translate-y-1/2 -z-10 transition-all duration-500 ease-out"
            style={{
              width:
                phase === 0
                  ? "0%"
...
function TimelineNode({
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 relative group">
      <div
        className={`w-3 h-3 rounded-full border-2 transition-all duration-300 flex items-center justify-center bg-background
        ${
          done
            ? "border-success bg-success/20 shadow-[0_0_8px_rgba(16,185,129,0.3)]"
            : active
              ? "border-accent bg-accent/20 shadow-[0_0_8px_rgba(255,59,59,0.3)]"
              : "border-border/60"
        }`}
      >
      ...
      <span
        className={`text-[9px] uppercase tracking-wider font-medium transition-colors absolute top-5 whitespace-nowrap
        ${
          done
            ? "text-success/80"
            : active
