...
export function PromptEditor({
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      textareaRef.current.style.height =
        Math.min(Math.max(scrollHeight, 180), 400) + "px"
    }
  }, [promptText])
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Tab") {
      ...
      const newValue =
        promptText.substring(0, start) + "  " + promptText.substring(end)
...
function ToolbarButton({
  ...
  return (
    <button className="flex items-center gap-1.5 px-2 py-1.5 rounded hover:bg-surface-2 text-xs text-muted hover:text-foreground transition-colors group whitespace-nowrap">
      <span className="text-muted/70 group-hover:text-foreground/80 transition-colors">
        {icon}
      </span>
      {label}
    </button>
...
function ToolbarToggle({
  ...
  return (
    <button
      className={`flex items-center gap-1.5 px-2 py-1.5 rounded text-xs transition-colors group whitespace-nowrap ${
        active
    ...
    >
      <span
        className={
          active
            ? "text-accent"
            : "text-muted/70 group-hover:text-foreground/80"
        }
      >
        {icon}
      </span>
