      ...
      className={`
        ...
        ${
          expanded
            ...
            : "border-border/30 hover:border-border hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] hover:bg-surface/60"
        }
      `}
      style={{
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <motion.div
              layoutId={`provider-icon-${model.id}`}
              className="shrink-0"
            >
              <ProviderIcon
                type={model.provider}
            ...
            <div className="flex flex-col">
              <motion.div
                layoutId={`model-name-${model.id}`}
                className="flex items-center gap-2"
              >
                <h3 className="font-bold text-foreground text-lg tracking-tight font-space">
                  {model.name}
                </h3>
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wider uppercase border ${statusColors[model.status]}`}
                >
                  {model.status}
                </span>
              </motion.div>
              <motion.div
                layoutId={`model-provider-${model.id}`}
                className="text-sm text-muted capitalize font-medium flex items-center gap-1.5 mt-0.5"
              >
                {model.provider} <span className="text-border/60">•</span>{" "}
                {model.version}
              </motion.div>
            </div>
          </div>
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors ${
                    isHealthy
                      ...
                      : "text-accent hover:text-accent hover:bg-accent/20"
                  }`}
                  aria-label={isHealthy ? "Disable Model" : "Enable Model"}
                >
                  <Power size={16} />
                </button>
                <button
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm text-foreground font-mono">
              ${model.inputCost} <span className="text-muted/60">/</span> $
              {model.outputCost}
            </span>
          </div>
          <div className="flex flex-col gap-1">
                      ...
                      <div
                        ...
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${
                          f.supported
                            ...
                            : "bg-surface/50 border-border/30 text-muted"
                        }`}
                      >
                        {f.supported ? (
                          <CheckCircle size={12} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-surface/50 rounded-lg p-3 border border-border/40">
                      ...
                      <span className="text-lg font-space text-foreground">
                        {model.successRate}%
                      </span>
                    </div>
                    <div className="bg-surface/50 rounded-lg p-3 border border-border/40">
                  ...
                  <div className="bg-surface/50 rounded-lg border border-border/40 overflow-hidden">
                    <div className="px-3 py-2 border-b border-border/40 bg-surface/80 flex items-center gap-2 text-xs text-muted">
                      <span className="font-mono text-accent bg-accent/10 px-1 rounded">
                        {model.endpoints[0]?.method}
                      </span>
                      <span className="font-mono truncate">
                        {model.endpoints[0]?.path}
                      </span>
                    </div>
