...
const generateData = (frame: TimeFrame) => {
  const points =
    frame === "Today" ? 24 : frame === "Week" ? 7 : frame === "Month" ? 30 : 12
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
        <div>
          ...
          <p className="text-xs text-muted mt-1">
            Provider cost breakdown over time
          </p>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Area
              ...
              key={`mistral-${timeFrame}-${refreshKey}`}
            />
            <Area
              type="monotone"
              dataKey="Azure"
              ...
              animationDuration={800}
              animationEasing="ease-out"
              key={`azure-${timeFrame}-${refreshKey}`}
            />
            <Area
              type="monotone"
              dataKey="Gemini"
              ...
              animationDuration={800}
              animationEasing="ease-out"
              key={`gemini-${timeFrame}-${refreshKey}`}
            />
            <Area
              type="monotone"
              dataKey="Claude"
              ...
              animationDuration={800}
              animationEasing="ease-out"
              key={`claude-${timeFrame}-${refreshKey}`}
            />
            <Area
              type="monotone"
              dataKey="OpenAI"
              ...
              animationDuration={800}
              animationEasing="ease-out"
              key={`openai-${timeFrame}-${refreshKey}`}
            />
          </AreaChart>
        </ResponsiveContainer>
      </div>
