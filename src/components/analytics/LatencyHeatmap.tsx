  ...
  const data = useMemo(() => {
    return PROVIDERS.map((p) => {
      return {
        ...
        hours: HOURS.map((h) => {
          const baseLatency =
            p.id === "anthropic" ? 400 : p.id === "openai" ? 250 : 300
      ...
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-xs text-muted">
            24-hour latency distribution across providers
          </p>
        </div>
          ...
          <div className="flex mb-2">
            ...
            <div className="flex-1 flex justify-between px-2">
              {HOURS.filter((_, i) => i % 4 === 0).map((h) => (
                <div key={h} className="text-[10px] text-muted/60">
                  {h}
                </div>
              ))}
            </div>
              ...
              <div className="flex-1 flex gap-1 h-6">
                {row.hours.map((cell, i) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    transition={{
                      delay: shouldReduceMotion ? 0 : i * 0.02 + 0.5,
                    }}
                    onMouseEnter={() =>
                      setHoveredCell({
                        p: row.provider.name,
        ...
        {hoveredCell && (
          ...
          >
            <div className="text-xs font-semibold text-foreground mb-1">
              {hoveredCell.p}{" "}
              <span className="text-muted font-normal">• {hoveredCell.h}</span>
            </div>
