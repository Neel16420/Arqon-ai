  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-bold text-foreground font-space">
            Model Leaderboard
          </h2>
          <p className="text-xs text-muted">
            Performance ranking across all active models
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="text-xs text-muted uppercase tracking-wider border-b border-border/40">
              <th
                className="pb-3 px-4 font-medium cursor-pointer hover:text-foreground transition-colors"
              ...
              >
                <div className="flex items-center gap-1">
                  Avg Latency <ArrowUpDown size={12} />
                </div>
              </th>
              <th
                className="pb-3 px-4 font-medium cursor-pointer hover:text-foreground transition-colors"
              ...
              >
                <div className="flex items-center gap-1">
                  Success Rate <ArrowUpDown size={12} />
                </div>
              </th>
              <th
                className="pb-3 px-4 font-medium cursor-pointer hover:text-foreground transition-colors"
              ...
              >
                <div className="flex items-center gap-1">
                  Cost / 1M <ArrowUpDown size={12} />
                </div>
              </th>
              <th
                className="pb-3 px-4 font-medium cursor-pointer hover:text-foreground transition-colors text-right"
                onClick={() => handleSort("requests")}
              >
                <div className="flex items-center justify-end gap-1">
                  Total Requests <ArrowUpDown size={12} />
                </div>
              </th>
            </tr>
          </thead>
            ...
            <AnimatePresence mode="popLayout">
              {sortedData.map((row, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-4 font-mono text-muted group-hover:text-foreground">
                    {row.latency}ms
                  </td>
                  <td className="py-4 px-4 font-mono text-muted group-hover:text-foreground">
                    {row.success}%
