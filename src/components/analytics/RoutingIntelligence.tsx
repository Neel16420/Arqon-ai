...
export function RoutingIntelligence({
  refreshTrigger,
}: RoutingIntelligenceProps) {
  const shouldReduceMotion = useReducedMotion()

  const fallbacks = useCountUp(
    useMemo(() => Math.floor(Math.random() * 200) + 500, [refreshTrigger]),
    shouldReduceMotion ? 0 : 1400,
  )

  const cacheHit = useCountUp(
    useMemo(() => Math.floor(Math.random() * 15) + 30, [refreshTrigger]),
    shouldReduceMotion ? 0 : 1400,
  )
    ...
    >
      <div>
        ...
        <p className="text-xs text-muted mb-6">
          Smart routing decisions made in the last 24h
        </p>
      </div>
