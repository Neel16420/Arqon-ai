...
export function MetricCards({ refreshTrigger }: MetricCardsProps) {
  ...
  const savingsValue = useCountUp(
    useMemo(() => Math.floor(Math.random() * 200) + 200, [refreshTrigger]),
  )
    ...
    >
      ...
      <div className="flex items-end justify-between relative z-10">
        <div className="text-2xl font-bold text-foreground font-space flex items-baseline">
          {card.prefix}
          {card.suffix === "M" ? (Number(count) / 1000).toFixed(2) : count}
          {card.suffix}
        </div>
