...
export function GlobalTimeline({ refreshTrigger }: GlobalTimelineProps) {
  ...
  const totalRequests = useCountUp(
    useMemo(
      () => Math.floor(Math.random() * 50000) + 2000000,
      [refreshTrigger],
    ),
    ...
    >
      <div className="shrink-0 flex items-center gap-4">
        <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center relative">
          ...
          <div
            className="absolute inset-0 rounded-full border border-accent/30 animate-ping opacity-20"
        ...
        <div>
          ...
          <div className="text-lg font-bold text-foreground font-space flex items-baseline gap-1">
            ...
            <span className="text-[10px] text-success font-medium tracking-wide">
              ▲ LIVE
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-end h-8 gap-0.5 w-full">
        {data.map((d, i) => (
          <motion.div
            ...
            initial={
              shouldReduceMotion
                ? { height: `${Math.max(10, d.intensity * 100)}%` }
                : { height: 0 }
            }
            ...
            className={`flex-1 rounded-t-sm ${
              d.intensity > 0.7 ? "bg-accent/80" : "bg-accent/30"
            }`}
          />
        ))}
      </div>
