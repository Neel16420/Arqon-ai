...
const generateData = (type: MetricType) => {
  ...
  return Array.from({ length: points }).map((_, i) => ({
    ...
    value: Math.max(
      0,
      Math.floor(base + Math.random() * variance * (Math.sin(i / 3) + 1)),
    ),
  }))
}
...
const CustomDot = (props: any) => {
  ...
  return (
    <motion.circle
      ...
      animate={{ scale: 1 }}
      transition={{
        delay: shouldReduceMotion ? 0 : 0.8 + index * 0.03,
        duration: 0.3,
        type: "spring",
      }}
...
export function PerformanceHero({ refreshTrigger }: PerformanceHeroProps) {
  ...
  const data = useMemo(
    () => generateData(activeMetric),
    [activeMetric, refreshTrigger],
  )
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
        <div>
          ...
          <p className="text-xs text-muted mt-1">
            Aggregated platform metrics over the last 24 hours.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <Area
              ...
              style={{
                ...
                animation: shouldReduceMotion
                  ? "none"
                  : "delayedFadeIn 300ms ease-out 800ms forwards",
              }}
            />
            ...
            <Line
              ...
              activeDot={(props: any) => (
                <CustomActiveDot {...props} color={strokeColor} />
              )}
            />
          </AreaChart>
        </ResponsiveContainer>
